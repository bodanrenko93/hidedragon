...
//	}
	...
	TWList::iterator cNode;
	for(cNode = sP->twlist.begin();cNode != sP->twlist.end(); cNode ++)
	{
...
int OnTabSelChange(int iPage)
{
	...
	case 5:
#endif
		HideOtherPage(6);//iPage);
		ShowHideIPPage();
		ChangeMenuState(6);

		break;
	case 6:
		HideOtherPage(7);//iPage);
		ShowAutoHidePage();
		ChangeMenuState(7);
...
int ShowWebPage()
{
	...
	LoadItemStringW(L"IDS_UNIONPAGE_DEFAULT",cURL,L"http://www.hidedragon.com/cn/union/union.html");


	DisplayHTMLPage(hStatic,cURL/*L"http://www.hidedragon.com/cn/union/union.html"*/);
	g_oleemb = 1;
...
int ShowCleanPage()
{
//	HWND hCleanPage = NULL;
//		hCleanPage = CreateDialog(hInst,MAKEINTRESOURCE(IDD_TAB_CLEANPRIV),
//			g_Tab,CleanPageProc);
//		g_TabWnd[5] = hCleanPage;
...
int ShowHideIPPage()
{
	...
	{	
// 		hHideIPPage = CreateDialog(hInst,MAKEINTRESOURCE(IDD_TAB_HIDEIP),
// 			g_Tab,HideIPPageProc);
// 		g_TabWnd[6] = hHideIPPage;
		UseProxyDlg dlg;
		dlg.create(g_Tab,hInst);
	}
	...
	case WM_SIZE:
		{
			...
			LONG btnLen = btnRect.right - btnRect.left;
			LONG adlinkLen = adlinkRect.right - adlinkRect.left;
			SetWindowPos(hStatic_ADLink,NULL,btnLen + 30,15,pageRect.right-pageRect.left - (btnLen + 50),
				20,SWP_NOZORDER);
	...
	case WM_SIZE:
		{
			...
			SetWindowPos(hAutoHideList,NULL,10,50,
				pageRect.right-pageRect.left - 20,pageRect.bottom-pageRect.top-119,SWP_NOZORDER);
			
			
			
// 			SetWindowPos(hAutoHide_Add,NULL,10,50,
// 				0,0,SWP_NOZORDER|SWP_NOSIZE);
// 			SetWindowPos(hAutoHide_Tip,NULL,5,3,
	...
	case WM_SIZE:
		{
			...
			LONG btnLen = btnRect.right - btnRect.left;
			LONG adlinkLen = adlinkRect.right - adlinkRect.left;
			SetWindowPos(hBK_ADLink,NULL,btnLen + 30,15,pageRect.right-pageRect.left - (btnLen + 50),
				20,SWP_NOZORDER);
...
int BuyHideDragon()
{
	wchar_t * pBuyUrl = new wchar_t[1024 * 3];
	memset(pBuyUrl,0,1024 * 3 * sizeof(wchar_t));
	
	regGetBuyURL(L"HideDragonSoftware", L"HideDragon", L"0",pBuyUrl);
	
	if (1)//wcscmp(pBuyUrl,L"") == 0)
	{
        // BuyURL doesn't exsits in registry, default it
		wchar_t * pLink = new wchar_t[1024];
		//		ShellExecute(NULL,L"open",
		//			LoadItemStringW(L"IDM_LINK_BUY",pLink,
		//			L"http://kinghome.51.net/en/BuyNow.htm"),NULL,NULL,SW_SHOW);
		ShellExecute(NULL,L"open",L"IEXPLORE",
			L"http://www.regnow.com/softsell/nph-softsell.cgi?item=17082-2&affiliate=338116",NULL,SW_SHOW);
		delete [] pLink;
    }
	else
	{
		ShellExecute(NULL,L"open",L"IEXPLORE",
			pBuyUrl,NULL,SW_SHOW);
		//		ShellExecute(NULL,L"open",L"IEXPLORE",pBuyUrl,NULL,NULL,SW_SHOW);
	}
...
int regGetBuyURL(wchar_t * publisher, wchar_t * appName, wchar_t * appVer,wchar_t * pBuyUrl )
{
	...
	wchar_t *  keyPath = new wchar_t[1024];
	memset(keyPath,0,1024 * sizeof(wchar_t));
	wcscpy(keyPath,L"SOFTWARE\\Digital River\\SoftwarePassport\\");
	wcscat(keyPath,publisher);
	wcscat(keyPath,L"\\");
	wcscat(keyPath,appName);
	wcscat(keyPath,L"\\");
	wcscat(keyPath,appVer);
	...
	if (lRet == ERROR_SUCCESS)
	{
        ...
        RegQueryValueEx(hKey, L"BuyURL",0, NULL, (LPBYTE)pBuyUrl, &dwSize);
