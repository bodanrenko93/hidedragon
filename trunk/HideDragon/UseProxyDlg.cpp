...
// 			L"http://www.store3000.com/hidedragon", TRUE);
				...
				{		
					HWND ads_adv = GetDlgItem(hWnd,IDC_HIDEIPADV_ADS);
					
					gif_adv.Attach(ads_adv);
					wchar_t cFileName[1024] = L"";
					GetCurrentDirectory_HD(1024,cFileName);
					wcscat(cFileName,L"\\gif\\gif_adv_def.gif");
					gif_adv.Load(cFileName);
					gif_adv.Draw();
...
//			return (LRESULT)g_brushGeneralPage;
		case WM_SETCURSOR:
			{
				POINT pMouse;
				HWND hwndGif = GetDlgItem(hWnd,IDC_HIDEIPADV_ADS);
				RECT rtGif;
				GetWindowRect(hwndGif,&rtGif);
				
				MapWindowPoints(GetDesktopWindow(),hWnd,(tagPOINT*)&rtGif,2);
		...
		case WM_SIZE:
			{
				...
				HWND ads_adv = GetDlgItem(hWnd,IDC_HIDEIPADV_ADS);
				SetWindowPos(ads_adv,NULL,10,5,
					LOWORD(lParam) - 20,60,SWP_NOZORDER);



				SetWindowPos(hGroup_Advanced,NULL,10,80,LOWORD(lParam) - 15,HIWORD(lParam) - 85,
				SWP_NOZORDER);
				...
				SetWindowPos(hAdv_List,NULL,19,150,LOWORD(lParam) - 35,HIWORD(lParam) - 235,
				SWP_NOZORDER);
				...
				/*HIWORD(lParam)-*/115,0,0,SWP_NOZORDER|SWP_NOSIZE);
				...
				SetWindowPos(hAdv_ADLink,NULL,(btnLen*2) + 50,110,LOWORD(lParam) - (btnLen + 50),
					20,SWP_NOZORDER);
...
// 			L"http://www.store3000.com/hidedragon", TRUE);
				...
				{
					HWND ads_gen = GetDlgItem(hWnd,IDC_HIDEIPGEN_ADS);
					
					gif_gen.Attach(ads_gen);
					wchar_t cFileName[1024] = L"";
					GetCurrentDirectory_HD(1024,cFileName);
					wcscat(cFileName,L"\\gif\\gif_gen_def.gif");
					gif_gen.Load(cFileName);
					gif_gen.Draw();
					//PostMessage(hWnd,WM_SIZE,NULL,NULL);
				}
...
//			return (LRESULT)g_brushGeneralPage;
		case WM_SETCURSOR:
			{
				POINT pMouse;
				HWND hwndGif = GetDlgItem(hWnd,IDC_HIDEIPGEN_ADS);
				RECT rtGif;
				GetWindowRect(hwndGif,&rtGif);
		...
		case WM_SIZE:
			{
				...
				HWND ads_tray = GetDlgItem(hWnd,IDC_HIDEIPGEN_ADS);
				...
				SetWindowPos(hGroup_HideIP,NULL,10,80,LOWORD(lParam) - 15,HIWORD(lParam) - 85,
				SWP_NOZORDER);
