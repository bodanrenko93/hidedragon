 ...
 LRESULT CALLBACK CClearDlg::MainDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
			...
			HWND hCleanAds = NULL;
			wchar_t cDefaultText [1024] = L"";
			wchar_t cDefaultLink [1024]= L"";
	switch (message)
	{
		case WM_INITDIALOG:
			g_brushCleanPage = CreateSolidBrush(GetSysColor(CTLCOLOR_DLG));

			OutputDebugString(L"InitMainDlg(hDlg);======================================");
			InitMainDlg(hDlg);
			OutputDebugString(L"			if(sOP.bCleanAfterHide == 1)");
			...
			OutputDebugString(L"HWND ads_clean = GetDlgItem(hDlg,IDC_CLEAN_ADS);");
			
			{		
				HWND ads_clean = GetDlgItem(hDlg,IDC_CLEAN_ADS);
				
				ShowWindow(ads_clean,FALSE);
#ifndef _NOADS

				gif_clean.Attach(ads_clean);
				wchar_t cFileName[1024] = L"";
				GetCurrentDirectory_HD(1024,cFileName);
				wcscat(cFileName,L"\\gif\\gif_clean_def.gif");
				gif_clean.Load(cFileName);
				gif_clean.Draw();
#endif
		...
		case WM_SIZE:
			{
...
#ifndef _NOADS
				SetWindowPos(hTree,NULL,10,105,
					pageRect.right-pageRect.left - 20,pageRect.bottom-pageRect.top-170,SWP_NOZORDER);


#else


				SetWindowPos(hTree,NULL,10,50,
					pageRect.right-pageRect.left - 20,pageRect.bottom-pageRect.top-109,SWP_NOZORDER);
				...
				HWND ads_clean = GetDlgItem(hDlg,IDC_CLEAN_ADS);
				SetWindowPos(ads_clean,NULL,10,5,
					pageRect.right-pageRect.left - 20,60,SWP_NOZORDER);

#ifndef _NOADS

				SetWindowPos(hCleanTip,NULL,10,75,
		...
		case WM_MOUSEMOVE:
			{
				...
				POINT pMouse;
				HWND hwndGif = GetDlgItem(hDlg,IDC_CLEAN_ADS);
				RECT rtGif;
				GetWindowRect(hwndGif,&rtGif);
				
				MapWindowPoints(GetDesktopWindow(),hDlg,(tagPOINT*)&rtGif,2);
				
				GetCursorPos(&pMouse);
				ScreenToClient(hDlg,&pMouse);
					

				if(PtInRect(&rc,pt)||PtInRect(&rc2,pt)/*||PtInRect(&rtGif,pMouse)*/)
				{
					if(!g_hCursorLink)
					{
						int i=::GetLastError();
...
/*******************初始化Tree控件***************/
	...
	wchar_t pText[1024];// = new wchar_t;
	...
	if(MyKey)
	{
		...
		::ZeroMemory(UrlBuf,255*sizeof(wchar_t));
		DWORD regsz=REG_SZ;
		size_temp=255*sizeof(wchar_t);
	...
	OutputDebugString(L"item.pszText=LoadItemStringW LIDS_CLEANPRIV_WINDOWSMEDIAPLAYER,===================");
